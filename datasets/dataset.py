...
from custom import process_and_generate_flow

class NPY_datasets(Dataset):
    def __init__(self, path_Data, config, model, args, train=True, num_frames=2, num_classes=2, device='cuda',Test = False):
    ...
    def __getitem__(self, indx):
        ...
        if os.path.exists(flow_path):
            ...
            flow_tensor = process_and_generate_flow(self.model, self.args, img1, img2)
            np.save(flow_path, flow_tensor)
            flow_tensor = torch.tensor(flow_tensor, dtype=torch.float32).to(self.device)
